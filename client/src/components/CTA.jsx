...
// Uses simple props to decide state; wire to real auth later

const CTA = ({ isLoggedIn = false, userName = "Reader" }) => {
  return (
    <section id="cta" className="py-16" style={{ backgroundColor: '#f8f9fa' }}>
      <div className="max-w-5xl mx-auto" style={{ padding: '0 24px' }}>
        ...
        <div className="text-center">
          ...
          <h2 className="text-black font-semibold mb-3" style={{ fontSize: isLoggedIn ? '32px' : '36px', lineHeight: 1.2 }}>
            {isLoggedIn ? `Welcome back, ${userName} 👋` : 'Step into the world of books 📚'}
          </h2>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            {isLoggedIn ? (
              <>
                ...
                <a href="#features" className="rounded-xl border border-[#0B6623] text-[#0B6623] hover:bg-[#0B6623] hover:text-white transition-colors" style={{ height: '48px', padding: '0 24px', minWidth: '180px', display: 'inline-flex', alignItems: 'center', justifyContent: 'center' }}>Explore Books</a>
              </>
            ) : (
              <>
